...
var negative1 = module.exports = exports = function(scarlet){
	...
	self.ensureCallback = function(target, methodName, defaultValues){
		...
		if(methodName)
			return scarlet.intercept(target,methodName).using(forceCallbackInterceptor).resolve();

		return scarlet.intercept(target).using(forceCallbackInterceptor).resolve();
	};
};